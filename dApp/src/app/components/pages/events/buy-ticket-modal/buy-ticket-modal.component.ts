    ...
    try{
      ...
      this.eventService.generateAndSubmitTicketMetadata(this.event!, this.ticketType.id, this.ticketType.name, this.ticketType.logo).subscribe((async (response: any) =>{
        ...
        if(price.currency != zeroAddress){
          ...
          console.log('allowance: ',allowance, ', price: ', price.price )
          if( allowance <   price.price){
        ...
        // })
        console.log('done arrpoving: ')

        
        const amount = +formatUnits(price.price, 18)
        const fee = amount * 0.05

        console.log('amount * 0.05: ', amount * 0.05)
        console.log('pric: ', amount)
